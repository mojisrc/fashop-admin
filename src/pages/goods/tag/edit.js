...
@connect(({ goodsTag, loading }) => ({
    ...
    goodsTagLoading: loading.effects["goodsTag/list"],
    goodsTagInfo: goodsTag.info.result,
    goodsTagInfoLoading: loading.effects["goodsTag/info"]
}))
export default class GoodsCategoryEdit extends Component {
    static defaultProps = {
        goodsTagLoading: true,
        goodsTag: {
            list: []
        },
        goodsTagInfoLoading: true,
        goodsTagInfo: {
            info: {}
        }
    ...
    render() {
        const { form, goodsTag, goodsTagLoading ,goodsTagInfoLoading} = this.props;
        ...
        const tree = Arr.toTree(goodsTag.list);
        // 最多3级 去掉自己
        let treeData = [];
        ...
        return (
            <PageHeaderWrapper hiddenBreadcrumb={true} policy={'goodscategory/edit'}>
                <Card bordered={false}>
                    <Spin size="large" spinning={goodsTagLoading||goodsTagInfoLoading}>
